...
class ResumenSolicitudPage extends StatelessWidget {
  ...
  // Parámetros para mostrar botón de chat desde tab de aceptadas
  ...
  bool get _isProviderView => solicitud != null;

  @override
  Widget build(BuildContext context) {
    // Si es vista de proveedor, usar datos de solicitud
    if (_isProviderView) {
      return _buildProviderView(context);
    }
    
    // Vista de cliente - usar RequestSummaryPage existente
    // Por ahora, redirigir a la vista de proveedor con datos convertidos
    // o mantener la estructura actual pero adaptada
    return _buildProviderView(context);
  }
  
  Widget _buildProviderView(BuildContext context) {
    // Obtener artículos según el tipo de vista
    final List<Map<String, dynamic>> articulos = _isProviderView
        ? (solicitud!['articulos'] as List<Map<String, dynamic>>? ?? [
            {
              'nombre': 'Televisión',
              'cantidad': 8,
              'peso': 30.8,
  ...
  // Convertir RequestItem a Map para compatibilidad
  List<Map<String, dynamic>> _convertItemsToMap() {
    ...
    return items!.map((item) => {
      ...
      'alto': item.height ?? 0.0,
      'ancho': item.width ?? 0.0,
      'largo': item.length ?? 0.0,
      'fragil': item.isFragile,
    }).toList();
  }
      ...
      child: Column(
        ...
        children: [
          if (_isProviderView) ...[
            ...
            _buildInfoRow('Nombre:', name ?? 'Sin nombre'),
            const SizedBox(height: 8),
            _buildInfoRow('Día:', date ?? 'No especificado'),
            const SizedBox(height: 8),
            _buildInfoRow('Origen:', '${originDist ?? ''}, ${originProv ?? ''}'),
            const SizedBox(height: 8),
            _buildInfoRow('Destino:', '${destDist ?? ''}, ${destProv ?? ''}'),
          ],
        ],
      ),
    );
